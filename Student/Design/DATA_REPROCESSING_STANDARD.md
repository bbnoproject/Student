...
- 무단/무연락 결석, 이탈면담, 경고면담 → `warning`
- 반복 미제출, 프로젝트 지연 제출, 짧은 주기의 컨디션 리듬 흔들림 → `caution`
- 단발 늦잠 지각은 태도형 위험이 아니라 `condition` 영향으로 분리하고, 최근성·반복 간격·프로젝트 구간을 함께 확인
- 일반 제출, 일반 회고, 일반 면담 → `stable`

### 6.4 복합 맥락 해석 규칙

- 같은 현상도 시간, 커리큘럼, 팀, 역할, 환경에 따라 다르게 해석한다.
- 늦잠 지각은 건강/컨디션 관리 신호로 분리하고, 최근 짧은 주기로 반복될 때 확인 필요 케이스로 만든다.
- 협업은 반복 팀원이나 선호/비선호 자체가 아니라 프로젝트 중 타 학생의 불만, 불화, 소통 저해, 회고와 해결 행동을 함께 본다.
- 그룹 경향은 탐색용이며, 학생별 판단은 개인의 변화, 회복, 상담, 팀 맥락을 반영해 보정한다.

### 6.5 자기표현 재가공 규칙

- 나이, 성별, 학력은 점수 근거가 아니라 해석 맥락으로만 저장한다.
- 모집서류, 대원카드, 데일리체크인, 회고, 자기소개서/이력서처럼 학생이 직접 작성한 문서를 별도 자기표현 프로파일로 재가공한다.
- 표현 특징은 구체성, 주도성 표현, 성찰 표현, 관계/협업 언어, 진로 언어, 정서/부담 표현, 탐색/불확실 표현으로 나눈다.
- 자기표현 프로파일은 학생 멘토링, 피드백 톤, 후속 질문 설계에 사용하고 단독 분류 근거로 쓰지 않는다.
